...
const testGetUserExercises = async (event) => {
    ...
    if (response.statusCode !== 200) {
        ...
        // Parse the body to retrieve the exercise data and any errors
        const responseBody = JSON.parse(response.body);
        const exercises = responseBody.exercises;
        ...
        const prettyExercisesValue = JSON.stringify(exercises, null, 2);
        const exerciseOutput = exercises.length > 0 ? prettyExercisesValue : messageValue;

        console.log(exerciseOutput);

        // Log any attribute retrieval errors
        exercises.forEach((exercise, index) => {
            if (exercise.attributeErrors && exercise.attributeErrors.length > 0) {
                console.log(`Attribute errors for exercise ${index + 1}:`, exercise.attributeErrors);
            }
        });
...
testGetUserExercises(eventNoUserID);
testGetUserExercises(eventInvalidUserID);
testGetUserExercises(eventEmpty);
